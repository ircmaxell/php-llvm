...
class Value implements CoreValue {
    ...
    public function __construct(LLVM $llvm, Context $context, LLVMValueRef $value, int $kind) {
        $this->llvm = $llvm;
    ...
    public function typeOf(): CoreType {
        return $this->llvm->factory->type($this->context, $this->llvm->lib->LLVMTypeOf($this->value));
    }
    ...
    public function getOperand(int $index): CoreValue {
        return $this->llvm->factory->value($this->context, $this->llvm->lib->LLVMGetOperand($this->value, $index));
    }

    public function getOperandUse(int $index): CoreUse {
        return $this->llvm->factory->use($this->context, $this->llvm->lib->LLVMGetOperandUse($this->value, $index));
    }
    ...
    public function getMetadata(int $kind): CoreValue {
        // Todo: map kind
        return $this->llvm->factory->value($this->context, $this->llvm->lib->LLVMGetMetadata($this->value, $kind));
    }
